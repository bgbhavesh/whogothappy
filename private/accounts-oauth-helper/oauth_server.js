...
Accounts.oauth._renderOauthResults = function(res, query) {
  ...
  if ('close' in query) { // check with 'in' because we don't set a value
    closePopup(res,query);
    // res.writeHead(302, {'Location': "http://174.129.12.79:3000/"});
    // res.end();
...
var closePopup = function(res,query) {
  ...
  res.writeHead(200, {'Content-Type': 'text/html'});
  var content =
        '<html style=" -webkit-backface-visibility: hidden; -webkit-transform: translateZ(0);-webkit-perspective:1000;-webkit-backface-visibility: hidden;"><title>  </title><head><script>'+"window.close();//window.open('http://youtap.meteor.com/', '_blank');" +' //window.location="http://174.129.12.79:3000"; window.location=window.localStorage.getItem("redirect")</script></head></html>';
