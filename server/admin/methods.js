Meteor.methods({
    "getPushId": function (options) {
        ...
        Meteor.users.update({"_id": Meteor.userId()}, {
            $set: {
                "profile.pushId": options.pushId,
                "profile.pushServer": "android"
            }
        });
            ...
            if (!cursor) {
                if (option.first)
                    ...
                    id = Streak.insert({
                        ...
                        "day": option.day,
                        "second": true,
                        "endgame2": option.endgame
                    });
            } else {
                if (option.first) {
                    if (option.endgame) {
                        id = Streak.update({"user": Meteor.userId(), "day": option.day}, {
                            $set: {
                                "first": true,
                                "endgame1": true
                            }
    ...
    "resetStreak": function () {
        ...
        if (Meteor.userId()) {
            ...
            // if(cursor){
            for (var i = 0; i < 5; i++) {
                Streak.update({"user": Meteor.userId(), "day": i}, {
                    $set: {
                        "first": false,
                        "endgame1": false,
                        "second": false,
