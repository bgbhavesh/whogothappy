...
Meteor.startup(function(){
	...
	// app.debug =false;
	if(app.debug){
		var lang = Language.findOne("lang_en");
		if(lang){
			app.language.en = lang;
			app.settings = lang.settings;
		}
		// console.log(lang);
		return;
	...
	Meteor.setTimeout(function(){
		...
		app.settings = {};
       	try{
       		return;
       		var my_sheet = new GoogleSpreadsheet('1gamLctiV_k8sSzOtxfJ543-Rjiw7RPIQo-Quihnkngs');
				...
				if(lan == "fr"){
					if(key[0] == "settings"){
						if(!app.settings[key[0]])
							app.settings[key[0]] = {};
						app.settings[key[0]][key[1]] = row_data[i].enm;
						// console.log(key[1])
					}
